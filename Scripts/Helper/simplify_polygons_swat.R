...
countries <- read_sf("Data/GIS Shapefile/Countries_Separated_with_associated_territories.shp") %>%
                      rename(GID_0 = ISO3CD, NAME_0 = MAP_LABEL) %>% subset(select=-c(OBJECTID,ROMNAM,M49Code))
...
countries3 <- st_simplify(countries, dTolerance=2000, preserveTopology = T) 
...
#write to file
setwd("Data")
if (dir.exists("cntrs_smpl_swat")==F) {dir.create("cntrs_smpl_swat")}
setwd("cntrs_smpl_swat")
